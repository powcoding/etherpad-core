...
function getCollabClient(ace2editor, serverVars, initialUserInfo, options, _pad)
{
  ...
  editorInfo.ace_setProperty("userAuthor", userId);
  //初始化rep，把服务器上的atext和apool设置到rep上
  editorInfo.ace_setBaseAttributedText(serverVars.initialAttributedText, serverVars.apool);
  //设置change事件的回调函数为handleUserChanges
  editorInfo.ace_setUserChangeNotificationCallback(wrapRecordingErrors("handleUserChanges", handleUserChanges));
  ...
  {
    if (editorInfo.ace_getInInternationalComposition()) 
    {
      return;
    }
    ...
    //如果当前时间还没有到达下一次发送时间，那么延迟到下一次发送时间
    if (t < earliestCommit)
    {
    ...
    if (msgQueue.length != 0) {
      ...
      while (msg = msgQueue.shift()) {
        ...
        else if (msg.type == "NEW_CHANGES")
        {
          ...
          editorInfo.ace_applyChangesToBase(changeset, author, apool);
        }
      }
    }
    ...
    //在这里，先执行了X<-Y，Y<-In
    var userChangesData = editorInfo.ace_prepareUserChangeset();
    //在这里，执行send Y to server
  ...
  function handleMessageFromServer(evt)
  {
    ...
    if (msg.type == "NEW_CHANGES")
    {
      ...
      if (msgQueue.length > 0 || editorInfo.ace_getInInternationalComposition()) {
  ...
  function tellAceAuthorInfo(userId, colorId, inactive)
  {
    ...
    // if (inactive)
    // {
    //   editorInfo.setAuthorInfo(userId, {
    //     bgcolor: cssColor,
    //     fade: 0.5
    //   });
    // }
