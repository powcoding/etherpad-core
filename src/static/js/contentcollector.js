...
function makeContentCollector(collectStyles, abrowser, apool, domInterface, className2Author)
{
  ...
  function _enterTitle(state, titleType) {
    var oldTitleType = state.lineAttributes['title'];
    if (titleType != 'none')
    {
      state.titleNesting = (state.titleNesting || 0) + 1;
    }
    
    if(titleType === 'none' || !titleType ){
      delete state.lineAttributes['title']; 
    }
    else{
      state.lineAttributes['title'] = titleType;
    }
  ...
  function _exitTitle(state, oldTitleType)
  {
    if (state.lineAttributes['title'])
    {
      state.titleNesting--;
    }
    if (oldTitleType && oldTitleType != 'none') { state.lineAttributes['title'] = oldTitleType; }
    else { delete state.lineAttributes['title']; }
    _recalcAttribString(state);
  }    
    ...
    else
    {
      ...
      else if (!isEmpty)
      {
        ...
        if (collectStyles)
        {
          ...
          if (tname == "u" || (styl && /\btext-decoration:\s*underline\b/i.exec(styl)) || (styl && /\btext-decoration-line:\s*underline\b/i.exec(styl)) || tname == "ins")
          {
            cc.doAttrib(state, "underline");
          }
          if (tname == "s" || (styl && /\btext-decoration:\s*line-through\b/i.exec(styl)) || (styl && /\btext-decoration-line:\s*line-through\b/i.exec(styl)) || tname == "del")
          {
            cc.doAttrib(state, "strikethrough");
          }
          if (tname == "h1" || tname == 'h2' || tname == "h3" || tname == "h4" || tname == "h5")
          {
            type = tname;
            oldTitleTypeOrNull = (_enterTitle(state, type) || 'none');
          }          
        ...
        if (oldTitleTypeOrNull)
        {
          _exitTitle(state, oldTitleTypeOrNull);
        }        
        if (oldAuthorOrNull)
        {
